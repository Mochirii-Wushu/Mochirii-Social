...
class AvatarService
{
    ...
    public static function cleanup($avatar, $confirm = false)
    {
        ...
        $curThumb = self::thumbnailName($curFile);

        $files = $files->filter(function ($f) use ($curFile, $curThumb) {
            return ! $curFile ||
                (
                    ! str_ends_with($f, $curFile) &&
                    ! str_ends_with($f, $curThumb)
                );
    ...
    public static function thumbnailName($file)
    {
        if (! $file) {
            return null;
        }
        ...
        return ($info['filename'] ?? $file).'_thumb'.$extension;
    }
}
