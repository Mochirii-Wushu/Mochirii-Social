							...
							<div class="col-sm-12">
								<div class="mb-1">
									<div class="form-check">
										...
										<label class="form-check-label font-weight-bold text-dark text-capitalize ml-1">Enable external sharing</label>
									</div>
								</div>
			...
			blockAction(action) {
				...
				swal({
					...
					content: {
						element: 'input',
						attributes: {
							placeholder: type == 'user' ? 'member' : 'social.mochirii.com'
					}
					},
					button: {
						text: "Next",
				...
				.then(name => {
					...
					if(action !== 'user' && name.startsWith('http')) {
						swal('Oops!', 'Please enter the Mochirii Social domain.', 'error');
						return null;
					}
					return name;
				}).then(name => {
